...
By default `cr_setup()` works under the forward formulation (i.e., the one we have simulated from). To fit the continuation ratio model under the bacward formulation, we would need to set `direction = "backward"` in the call to `cr_setup()`.
...
plot_data_m2 <- effectPlotData(fm, nDF, CR_cohort_varname = "cohort", marginal = TRUE)
```
